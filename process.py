...
if time_stamp_flag:
    for i in range(len(no_of_rows)):
        ...
        for k,v in all_sheets_dict.items():
            ...
            try:
                ...
                if current_chat in gibberish_question_list:
                    ...
                    # reschedule_replies = reschedule_sheet.iloc[:,1].tolist()
                    # random_reschedule_reply = random.choice(reschedule_replies)
        ...
        # worksheet=workbook.add_worksheet(f'sheet{tmp}')
        # writer.sheets[f'sheet{tmp}'] = worksheet
        # all_df_list.to_excel(writer, sheet_name=f'sheet{tmp}')
        # tmp = tmp+1
